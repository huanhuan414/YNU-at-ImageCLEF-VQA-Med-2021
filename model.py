...
class BAN_Model(nn.Module):
    def __init__(self, dataset,args):
        ...
        #torch.nn.init.constant_(self.w,0.5)
        self.args = args
        ...
        if args.other_model:
            self.unet = FixVgg16()
            self.convertu = nn.Linear(1984, 128)
    ...
    def forward(self, v, q):
        ...
        if self.args.autoencoder:
            ...
            ae_v_emb = encoder.view(encoder.shape[0], -1) #[B 16384]
            ae_v_emb = self.convert(ae_v_emb).unsqueeze(1) #[B 1 64]
            v_emb = ae_v_emb
        if self.args.maml and self.args.autoencoder:
            v_emb = torch.cat((maml_v_emb, ae_v_emb), 2)
        if self.args.other_model:
            other_v_emb = self.unet(v)  #input: b,c,h,w c==3 ; output= b,1,1,1984
            v_emb = other_v_emb.squeeze(2) # b,1,1984
            # v_emb = self.convertu(v_emb)# b,1,128
            # v_emb = v_emb*self.w + other_v_emb*(1-self.w)
